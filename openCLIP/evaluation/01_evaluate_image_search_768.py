"""
003_evaluate.py — OpenCLIP Top-1 / Top-5 Evaluation
=====================================================
...
Usage
-----
    python 003_evaluate.py
    python 003_evaluate.py --cpu
    python 003_evaluate.py --batch-size 64
"""
...
OUTPUT_PATH = Path("/app/results_003.json")
...
def embed_images_batch(model, preprocess, pil_images: list, device: torch.device) -> np.ndarray:
    """Embed a batch of PIL images → [N, 768] normalised float32."""
...
def evaluate(args):
    ...
    # per class: top1/top5 hits + confusion counter (what was predicted when wrong)
    per_class = defaultdict(lambda: {
        "top1_correct": 0,
        "top5_correct": 0,
        "total": 0,
        "confused_as": defaultdict(int),   # predicted_label -> count
    })
    errors = []
    ...
    def process_batch(paths, true_labels):
        ...
        for i, true_label in enumerate(valid_labels):
            ...
            # record confusion when top-1 is wrong
            if not hit1:
