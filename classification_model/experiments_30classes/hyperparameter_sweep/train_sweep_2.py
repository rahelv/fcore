...
sweep_config = {
    ...
    "parameters": {
        ...
        "rotation_degrees": {
            "values": [0, 5, 10, 20]  # 0 = effectively off
        },

        # ── augmentation: color ────────────────
        # instead of color_jitter_strength: ["none", "mild", "strong"]
        "jitter_brightness": {
...
def build_transforms(cfg):
    ...
    if cfg.jitter_brightness > 0 or cfg.jitter_saturation > 0:
        pre_tensor.append(
            transforms.ColorJitter(
                brightness=cfg.jitter_brightness,
                contrast=cfg.jitter_brightness,
                saturation=cfg.jitter_saturation,
                hue=cfg.jitter_saturation * 0.2,
            )
        )

    if cfg.grayscale_p > 0:
        pre_tensor.append(transforms.RandomGrayscale(p=cfg.grayscale_p))
    ...
    train_tf = transforms.Compose(pre_tensor + to_tensor)
