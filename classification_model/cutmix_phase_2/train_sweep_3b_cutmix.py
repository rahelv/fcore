...
CutMix
──────
Cuts a random rectangular patch from one training image and pastes it onto
another.  Labels are blended proportionally to the patch area:

    loss = lam * CE(output, label_a) + (1 - lam) * CE(output, label_b)

where lam = 1 - (patch_area / total_area).

The alpha parameter controls mixing strength via a Beta(alpha, alpha) distribution:
  - alpha 0.2  →  lam stays close to 1.0  →  small patch, mild mixing
  - alpha 1.0  →  lam ~ Uniform(0,1)       →  large patches, aggressive mixing

Usage
─────
    python3 create_sweep_3b_cutmix.py
    CUDA_VISIBLE_DEVICES=0 python3 run_agent_3b_cutmix.py <sweep_id> 25
    CUDA_VISIBLE_DEVICES=1 python3 run_agent_3b_cutmix.py <sweep_id> 25
"""
...
SAVE_DIR    = Path("/home/ubuntu/data/models/fcore_hyperparam_sweep_3b_cutmix")
IMAGE_SIZE  = 224
...
sweep_config = {
    ...
    "parameters": {
        ...
        "cutmix_alpha": {
            "values": [0.2, 0.4, 0.6],
            # Controls Beta(alpha, alpha) distribution for lambda sampling.
            # 0.2 → mild mixing (small patches)
            # 1.0 → aggressive mixing (large patches, uniform lambda)
        },
